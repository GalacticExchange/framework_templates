...
public class SinkCassandra implements Sink {
    ...
    SinkCassandra(String topic, String name) throws Exception {
        ...
        this.propertiesFilePath = Paths.get(Enchilada.configDir, name + "-cassandra.properties").toString();
        this.query = "INSERT INTO " + topic + " SELECT * FROM " + topic;
        this.username = Enchilada.properties.getProperty(PropertiesReader.CASSANDRA_USERNAME);
        this.password = Enchilada.properties.getProperty(PropertiesReader.CASSANDRA_PASSWORD);
        this.keyspace = Enchilada.properties.getProperty(PropertiesReader.CASSANDRA_KEYSPACE);
        this.table = validateTableName(topic);
        this.host = Enchilada.properties.getProperty(PropertiesReader.CASSANDRA_HOST);
        this.port = Integer.valueOf(Enchilada.properties.getProperty(PropertiesReader.CASSANDRA_PORT));
        checkConfig();
    }
